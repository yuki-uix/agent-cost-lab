...
这意味着 1.1 给出的是**条件分布**——「在非 compaction 的断裂里，谁占大头」。任何引用
1.1 的地方都必须带上这个条件。

**compaction 的占比未知，这里不做猜测。** 三份 capture 至今观测到 5 次真实断裂，
成因是 `cache_control` 的 ttl 抖动（4 次）和一处 system prompt 文本改动（1 次）——
**没有一次来自 survey 列的六个「缓存杀手」**，而占多数的那个成因根本不在那张表上。
证据目前指向别处，不指向 compaction。
